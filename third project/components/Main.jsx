...
export default function Main(){
    ...
    const mappedIngredient = addIngredient.map(list => 
        <li>{list}</li>
    )
    function toggleRecipeButton(){
    ...
    return(
        <main>
            ...
            {addIngredient.length > 0 && <section>
                <h2>Ingredients on hand:</h2>
                <ul className="ingredients-list" aria-live="polite">{mappedIngredient}</ul>

                {addIngredient.length > 3 && <div className="get-recipe-container">
                    <div>
                        <h3>Ready for a recipe?</h3>
                        <p>Generate a recipe from your list of ingredients.</p>
                    </div>
                    <button onClick={toggleRecipeButton}>Get a recipe</button>
                </div>}
